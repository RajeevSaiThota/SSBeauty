...
import com.sslweb.automation.test.page.actions.helper.SSBCheckoutPageWalletHelper;
import com.techouts.sslweb.webelement.ops.WebElementOperationsWeb;

public class SSBCheckoutPageWalletAction extends GlobalExceptionHandler {


	private WebDriver driver = null;
	public JavascriptExecutor js;

	private SSBCheckoutPageWalletHelper ssbcheckoutwalletpay;
	private SSBCheckoutPageNavigationHelper ssbcheckoutNavigation;

	public SSBCheckoutPageWalletAction(WebDriver driver, ExcelRepository repository) {
		this.driver = Objects.requireNonNull(driver,
				"WebDriver cannot be null to perform actions in AllFieldsDisplayedActions class");
		ssbcheckoutwalletpay = new SSBCheckoutPageWalletHelper(driver, repository);
	...
	public void EnteringWalletDetails(String testCaseName, String token) {
		try {
			WebElementOperationsWeb.waitForPageLoad(driver, 60);
			WebElementOperationsWeb.park(15);
			ssbcheckoutwalletpay.ClickonWalletCheckbox();
			WebElementOperationsWeb.park(20);
			// ssbcheckoutwalletpay.SendWalletOTP(testCaseName, token); // Must Implement OTP method
			WebElementOperationsWeb.park(3);
			ssbcheckoutwalletpay.ClickonVerify();
